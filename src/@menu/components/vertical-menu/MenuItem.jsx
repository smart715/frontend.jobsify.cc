...
const MenuItem = (props, ref) => {
  ...
  const href = rest.href || (component && typeof component !== 'string' && component.props?.href) || null

  const isGroupLabel = !href // null or ''

  const finalComponent = isGroupLabel ? 'div' : component
    ...
    >
      <MenuButton
        className={classnames(menuClasses.button, {
          [menuClasses.active]: active,
          'group-label': isGroupLabel
        })}
        component={finalComponent}
        tabIndex={-1}
        {...(!isGroupLabel ? rest : {})} // do not spread href or click handlers for group label
        onClick={e => {
          if (!isGroupLabel) {
            handleClick()
            rest.onClick?.(e)
        ...
        style={isGroupLabel ? { pointerEvents: 'none', cursor: 'default' } : undefined}
      >
        {/* Menu Item Icon */}
        {renderMenuIcon({
          icon,
