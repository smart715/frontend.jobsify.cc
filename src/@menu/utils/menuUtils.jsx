...
export const confirmUrlInChildren = (children, url) => {
  ...
  if (isValidElement(children)) {
    ...
    const cleanUrl = (urlString) => {
      if (!urlString) return ''
      return urlString.replace(/^\/[a-z]{2}\//, '/')
    }

    const cleanCurrentUrl = cleanUrl(url)

    if (component && component.props.href) {
      const cleanHref = cleanUrl(component.props.href)
      if (exactMatch === false && activeUrl) {
        const cleanActiveUrl = cleanUrl(activeUrl)
        return cleanCurrentUrl.includes(cleanActiveUrl) || url.includes(activeUrl)
      }
      return exactMatch === true || exactMatch === undefined
        ? cleanHref === cleanCurrentUrl || component.props.href === url
        : cleanCurrentUrl.includes(cleanHref)
    }
    ...
    if (href) {
      ...
      return exactMatch === true || exactMatch === undefined 
        ? cleanHref === cleanCurrentUrl || href === url 
        : cleanCurrentUrl.includes(cleanHref)
    }
